...
# Removing unnecessary data frames to clean up memory on system
rm(freq_df)
sqldf('select CONTACT_WID, sum(AMOUNT) as xAmount from  group by CONTACT_WID')
...
temp1 <- aggregate(cust_gam_actv_filtered$ACT_TME_SPN_QTY, by=list(cust_gam_actv_filtered$CONTACT_WID, df$X_GAME_NM), sum)
colnames(temp1) <- c('CONTACT_WID','X_GAME_NM','SourceWise')
...
write.csv(cust_final11, "cust_final11.csv", row.names = F)
...
# Removing final all data frames to clean up memory on system

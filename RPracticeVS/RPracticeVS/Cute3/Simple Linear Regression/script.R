...
head(rawFiltered$TotalRevenueGenerated)
...
xTrain <- subset(train_data, select = -c(TotalRevenueGenerated))
xtest <- subset(test_data, select = -c(TotalRevenueGenerated))
#install.packages("geoR")
#library(geoR)
# Running Linear Regression Model on filtered data
Lin_Reg <- train(xTrain, train_data$TotalRevenueGenerated,'lm', trControl = trainControl(method = 'cv', number = 15))
...
error_df <- data.frame(kfold_Train = regr.eval(train_data$TotalRevenueGenerated,predict(Lin_Reg$finalModel, xTrain)))
...
test_prediction <- predict(Lin_Reg$finalModel, xtest)

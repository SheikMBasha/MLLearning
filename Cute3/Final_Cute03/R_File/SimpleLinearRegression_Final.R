...
##############################################################
aicFilteredData <- subset(rawFiltered, select = -c(MAX_AGE, NumHouseChildren, RecencyApp360, Recencydown, Recencydown360, maxRecencyCum, StrengthOfFavoriteGame))
set.seed(9983)
datapart1 <- createDataPartition(aicFilteredData$TotalRevenueGenerated, times = 1, p = 0.7, list = F)
trainaic_data = aicFilteredData[datapart1,]
testaic_data = aicFilteredData[-datapart1,]

AICLin_Reg <- lm(TotalRevenueGenerated ~ ., data = trainaic_data)

error_df <- data.frame(error_df, AICLinReg_Train = regr.eval(trainaic_data$TotalRevenueGenerated, AICLin_Reg$fitted.values))

# Removing y = TotalRevenueGenerated from test_data and put in target
target1 <- testaic_data$TotalRevenueGenerated
testaic_data$TotalRevenueGenerated <- NULL

# Calculate Error metric on Test data for Linear Regression Model
test_prediction1 <- predict(AICLin_Reg, testaic_data)
error_df <- data.frame(error_df, AICLinReg_Test = regr.eval(target1, test_prediction1))

#Applying Tuki's transformation on AIC filtered data

aicFilteredData$TotalRevenueGeneratedT <- log(aicFilteredData$TotalRevenueGenerated)

rawFilterdTransform <- subset(aicFilteredData, select = -c(TotalRevenueGenerated))
...
error_df <- data.frame(error_df, LinReg_TrainTransform = regr.eval(train_data_T$TotalRevenueGeneratedT, Lin_Reg1$fitted.values))
...
error_df <- data.frame(error_df, LinReg_TestTransform = regr.eval(target_T, test_prediction_T))

...
library(DMwR)
Error_frame <- data.frame(Train3_700 = regr.eval(train_target, pred_train))
Error_frame <- data.frame(Error_frame, Test3_700 = regr.eval(test_target, pred_test))


model1 = xgb.train(data = dtrain, max.depth = 2,
                  eta = 0.3, nthread = 2, nround = 1000,
                  watchlist = watchlist,
                  eval.metric = "rmse",
                  objective = "reg:linear", verbose = 1)

importance1 <- xgb.importance(feature_names = names(train_Data), model = model1)
print(importance1)
xgb.plot.importance(importance_matrix = importance1)

pred_train1 <- predict(model1, as.matrix(train_Data))
pred_test1 <- predict(model1, as.matrix(test_Data))

Error_frame <- data.frame(Error_frame, Train2_1000 = regr.eval(train_target, pred_train1))
Error_frame <- data.frame(Error_frame, Test2_1000 = regr.eval(test_target, pred_test1))
